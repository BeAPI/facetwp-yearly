...
class Helpers {
	...
	public static function locate_template( $tpl ) {
		...
		$path = apply_filters( 'BEA/Helpers/locate_template/templates', array( 'views/' . BEAPI_FACETWP_YEARLY_VIEWS_FOLDER_NAME . '/' . $tpl . '.php' ), $tpl, __NAMESPACE__ );
		...
		// Locate on the files
		if ( is_file( BEAPI_FACETWP_YEARLY_DIR . 'views/' . $tpl . '.php' ) ) {// Use builtin template
			return ( BEAPI_FACETWP_YEARLY_DIR . 'views/' . $tpl . '.php' );
		}
